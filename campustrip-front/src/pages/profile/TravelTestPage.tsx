...
import { updateUserPreference } from "../../api/users";
...
interface TestResultPageProps {
  choices: (1 | 0 | null)[];
  onSave: (choices: (1 | 0 | null)[]) => Promise<void>; // 비동기 처리를 위해 Promise 반환으로 변경
  onBack: (isCompleted?: boolean) => void;
}

interface TravelTestFlowProps {
  onComplete: (choices: (1 | 0 | null)[]) => Promise<void>;
}
...
const QUIZ_QUESTIONS: Question[] = [
  // 1.계획 vs 즉흥
  {
    ...
    options: [
      {
        text: "미리 짜놓은 일정에 맞춰 바로 나선다.",
        choiceValue: 1,
        tag: TAG_PAIRS[0].high,
      },
      {
        text: "일단 늦잠자고, 일어나서 뭘 할지 생각해본다.",
        choiceValue: 0,
        tag: TAG_PAIRS[0].low,
      },
    ],
  ...
  // 2.맛집 vs 아무거나
  {
    ...
    options: [
      {
        text: "여행 전 맛집 리스트를 미리 저장해두고 방문한다.",
        choiceValue: 1,
        tag: TAG_PAIRS[1].high,
      },
      {
        text: "가장 가까운 곳에 방문하거나 간단히 해결한다.",
        choiceValue: 0,
        tag: TAG_PAIRS[1].low,
      },
    ],
  ...
  // 3.사진 vs 눈으로 기억
  {
    ...
    options: [
      {
        text: "인생샷을 건질 때까지 다양한 각도로 사진을 찍는다.",
        choiceValue: 1,
        tag: TAG_PAIRS[2].high,
      },
      {
        text: "폰은 내려놓고 눈과 마음에 담아두려 노력한다.",
        choiceValue: 0,
        tag: TAG_PAIRS[2].low,
      },
    ],
  ...
  // 4.뚜벅이 vs 교통 중심
  {
    ...
    options: [
      {
        text: "많이 걷더라도 도보로 구석구석 돌아다닌다.",
        choiceValue: 1,
        tag: TAG_PAIRS[3].high,
      },
      {
        text: "대중교통이나 택시를 이용해 편하게 이동한다.",
        choiceValue: 0,
        tag: TAG_PAIRS[3].low,
      },
    ],
  ...
  // 5. 가성비 vs 투자
  {
    ...
    options: [
      {
        text: "돈을 최대한 아끼기 위해 여러 곳을 비교한다.",
        choiceValue: 1,
        tag: TAG_PAIRS[4].high,
      },
      {
        text: "돈을 좀 더 쓰더라도 편리함과 쾌적함을 우선한다.",
        choiceValue: 0,
        tag: TAG_PAIRS[4].low,
      },
    ],
  ...
  // 6. 느긋힐링 vs 액티비티
  {
    ...
    options: [
      {
        text: "여유롭게 즐기며 휴식이 필수다.",
        choiceValue: 1,
        tag: TAG_PAIRS[5].high,
      },
      {
        text: "활동은 최대한 많이, 꽉찬 일정이 좋다.",
        choiceValue: 0,
        tag: TAG_PAIRS[5].low,
      },
    ],
  ...
  // 7. 규칙적 식사 vs 비규칙적 식사
  {
    ...
    options: [
      {
        text: "삼시세끼 정해진 시간에 챙겨먹는다.",
        choiceValue: 1,
        tag: TAG_PAIRS[6].high,
      },
      {
        text: "놀다가 배고파질 때 먹는다.",
        choiceValue: 0,
...
// 선택지를 3진법 정수로 변환 (백엔드 저장용)
// null(건너뛰기) -> 0
// 0(Low) -> 1
// 1(High) -> 2
const encodePreferences = (choices: (1 | 0 | null)[]): number => {
  let preference = 0;
  choices.forEach((choice, index) => {
    let val = 0;
    if (choice === 0) val = 1;
    if (choice === 1) val = 2;
    // choice === null 이면 val = 0 (건너뛰기)

    // 3^index 자리에 값 더하기
    preference += val * 3 ** index;
  });
  return preference;
};
...
const QuestionnairePage: React.FC<QuestionnairePageProps> = ({
  ...
  return (
    <TestPageContainer>
      <TestHeader>
        <TestIconButton onClick={() => onBack(false)}>
          <IoArrowBack />
        </TestIconButton>
        <span style={{ fontWeight: "600" }}>
          {currentQuestionIndex} / {totalQuestions}
        </span>
        <TestIconButton onClick={onSkip}>
          <span style={{ fontSize: "14px", fontWeight: "500" }}>건너뛰기</span>
        </TestIconButton>
      </TestHeader>
      <ProgressContainer>
    ...
    <TestPageContainer>
      ...
      <QuestionSection>
        <ResultCard>
          ...
          <SaveButton onClick={handleSaveAndExit} disabled={isSaving}>
            {isSaving ? "저장 중..." : "프로필에 저장하기"}
          </SaveButton>
        </ResultCard>

        <SkipLink onClick={() => navigate(-1)}>
          저장하지 않고 프로필로 돌아가기
        </SkipLink>
      </QuestionSection>
    </TestPageContainer>
...
const TravelTestFlow: React.FC<TravelTestFlowProps> = ({ onComplete }) => {
  ...
  const currentQuestion =
    currentQuestionIndex > 0 ? QUIZ_QUESTIONS[currentQuestionIndex - 1] : null;
  ...
  if (currentQuestionIndex === 0) {
    ...
    return (
      <TestPageContainer style={{ paddingTop: "80px", textAlign: "center" }}>
        ...
        <ResultDescription style={{ marginBottom: "40px" }}>
          간단한 질문으로 당신의 여행 스타일을 확인하세요.
        </ResultDescription>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <SkipLink onClick={() => navigate(-1)} style={{ color: "#9ca3af" }}>
            나중에 할게요
          </SkipLink>
        </div>
      </TestPageContainer>
...
// 최상위 Page 컴포넌트: AuthContext 연동 및 API 호출 담당
const TravelTestPage: React.FC = () => {
  const navigate = useNavigate();
  const { user } = useAuth();

  const handleComplete = async (choices: (1 | 0 | null)[]) => {
    if (!user) {
      alert("로그인이 필요합니다.");
      navigate("/login");
      return;
    ...
    const preferenceScore = encodePreferences(choices);

    try {
      await updateUserPreference(user.id, preferenceScore);
      // 저장 후 로직은 TestResultPage에서 navigate 처리됨
    } catch (error) {
      console.error("Failed to save preferences", error);
      alert("저장에 실패했습니다. 다시 시도해주세요.");
      throw error; // 에러를 던져서 TestResultPage에서 로딩 상태를 해제할 수 있게 함
    }
  };

  return <TravelTestFlow onComplete={handleComplete} />;
};
